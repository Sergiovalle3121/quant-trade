...
- A Stripe session unlocks only when it is `paid` in USD for at least the plan's price and carries metadata `app=rigor`; never treat `no_payment_required` or another link on the same Stripe account as payment.
