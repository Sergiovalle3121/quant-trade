...
def test_both_languages_cover_the_four_classes_and_pass_the_guard() -> None:
    for locale in ("es", "en"):
        ...
        assert html.count("class='rung you'") == 1
        assert "class='rung-cls'>B</span>" in html.split("class='rung you'")[1][:120]
...
def test_the_ladder_is_shown_before_payment_and_on_the_sample() -> None:
    ...
    marked = locked.split("class='rung you'")[1][:120]
    assert f"class='rung-cls'>{result.verdict.overall}</span>" in marked
