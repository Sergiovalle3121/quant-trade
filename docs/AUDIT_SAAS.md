...
5. Retention: run `quant-trade audit purge --days 30` and confirm with
   `--yes` (a Railway cron service with the same variables works with
   Postgres; with a SQLite volume run it from `railway ssh`, since a volume
   attaches to one service only). Unpaid uploads, reports and the
