...
def calculate_performance(equity_curve: pd.DataFrame, trades: list[Trade]) -> dict[str, Any]:
    ...
    volatility = float(returns.std(ddof=0) * math.sqrt(TRADING_DAYS)) if len(returns) > 1 else 0.0
    downside = returns[returns < 0]
    downside_vol = (
        float(downside.std(ddof=0) * math.sqrt(TRADING_DAYS)) if len(downside) > 1 else 0.0
    )
    sharpe = float((returns.mean() * TRADING_DAYS) / volatility) if volatility > 0 else 0.0
    sortino = float((returns.mean() * TRADING_DAYS) / downside_vol) if downside_vol > 0 else 0.0
